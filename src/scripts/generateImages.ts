...
const characterData = fs.readFileSync(
  path.resolve(__dirname, "../data/characters-raw.txt"),
  "utf8"
);
...
(async () => {
  for (let fontIndex = 0; fontIndex < fonts.length; fontIndex++) {
    ...
    const { canvas: familyCanvas, ctx: familyCtx } = printFamily(
      font,
      vectorSize,
      characters
    );
    ...
    console.log(
      `Family measured as ${font.familyName}-family.png minY: ${minY}, maxY: ${maxY}`
    );
    for (
      ...
      for (let i = 0; i < character.length; i++) {
        if (
          !font.hasGlyphForCodePoint(character.charCodeAt(i)) &&
          character !== " "
        ) {
          console.log(
            ` --- Glyph does not exist in this font. Skipping character ${character}.`
          );
          skip = true;
          break;
        }
      ...
      console.log(
        `Generating image for character ${character} with font ${font.familyName}`
      );
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      console.log(
        `registering font ${font.familyName} as font ${font.familyName}`
      );
      ...
      const characterDir = path.resolve(
        __dirname,
        `../data/characters/${sanitizedCharacter}`
      );
      ...
      const characterPath = path.resolve(
        characterDir,
        `${sanitizedCharacter}_${fontIndex}.png`
      );
      await writeImageFile(canvas, characterPath);
      fs.appendFileSync(
        path.resolve(__dirname, "../data/labels.csv"),
        `${character}\t${path.resolve(
          characterDir,
          `${sanitizedCharacter}_${fontIndex}.png\n`
        )}`
      );
