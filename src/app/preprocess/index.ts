...
export const invertIfDarkBackground = (
  canvas: Canvas,
...
export const convertToGreyscale = (
  canvas: Canvas,
  ctx: CanvasRenderingContext2D
) => {
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
...
export const centerImageToVectorSize = (
  canvas: Canvas,
  ctx: CanvasRenderingContext2D
) => {
  const originalWidth = canvas.width;
...
export const cropToHorizontalBounds = (
  canvas: Canvas,
  ctx: CanvasRenderingContext2D
) => {
  const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
  ...
  croppedCtx.drawImage(canvas, minX - 1, 0, width, height, 0, 0, width, height);
...
export const prepareSegment = (
  ...
  const imageData = segmentCtx.getImageData(
    0,
    0,
    segmentCanvas.width,
    segmentCanvas.height
  );
  //fs.writeFileSync(`./segments/segment_${name}.png`, segmentCanvas.toBuffer());
  const data = imageData.data;
